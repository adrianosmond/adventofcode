...
const manhattan3d = (from: number[], to: number[]): number =>
  Math.abs(from[0] - to[0]) +
  Math.abs(from[1] - to[1]) +
  Math.abs(from[2] - to[2]);

const inRangeOfBot = ([fromX, fromY, fromZ, range]: number[]): number =>
  bots.reduce(
    (tot, [toX, toY, toZ]: number[]) =>
...
const inRangeOfPt = ([fromX, fromY, fromZ]: number[]): number =>
  bots.reduce(
    (tot, [toX, toY, toZ, range]: number[]) =>
      manhattan3d([fromX, fromY, fromZ], [toX, toY, toZ]) <= range
        ? tot + 1
        : tot,
    0,
