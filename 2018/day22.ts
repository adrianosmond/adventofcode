...
const getOtherTool = (terrain: number, currentTool: number): number => {
  // rocky
  if (terrain === 0) {
...
const isToolAllowed = (tool: number, terrain: number): boolean =>
  terrain === tool || (terrain + 2) % 3 === tool;

const getAdjacentSquares = (current: Position): Position[] => {
  const adj: Position[] = [];
  const currTerrain = type[current.row][current.col];
  const testAndAdd = (row: number, col: number, tool: number): void => {
...
const findPath = (from: Position, to: Position): number => {
  ...
  while (openSet.length > 0) {
    ...
    if (!current) throw new Error('No current position found');
