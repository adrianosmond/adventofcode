...
const fives: Record<number | string, string> = {};
chars.forEach((c) => {
  fives[c] = new Array(5).fill(c).join('');
});

const hasFive = (hash: string, digit: string | number) =>
  hash.includes(fives[digit]);

const findKeys = (numHashes = 1) => {
  let i = 0;
  const indexes = [];
  const lastSeen = makeArrays(() => -1);
  const potentials = makeArrays<{ i: number; triple: string; hash: string }[]>(
    () => [],
  );
  let max = 0;
