...
for (let i = 0; i < positions.length; i++) {
  for (let j = i + 1; j < positions.length; j++) {
    ...
    if (distance === undefined)
      throw new Error(`No path between positions ${i} and ${j}`);
...
const getBestDistanceForRoute = (routes: number[][]): number => {
  let bestDistance = Number.MAX_SAFE_INTEGER;

  routes.forEach((route: number[]) => {
    let prev = 0;
    let distance = 0;
    route.forEach((stop: number) => {
...
export const part1 = () => getBestDistanceForRoute(permutator(toVisit));
