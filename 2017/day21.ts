...
const makeOutput = (inp: string[][]): string[][] =>
  new Array(inp.length).fill(null).map(() => new Array(inp[0].length).fill(''));

const flipHorizontal = (inp: string[][]): string[][] => {
...
const flipVertical = (inp: string[][]): string[][] => {
  const output = makeOutput(inp);
  for (let i = 0; i < inp.length; i++) {
...
const rotate90 = (inp: string[][]): string[][] => {
  const output = makeOutput(inp);
  for (let i = 0; i < inp.length; i++) {
    for (let j = 0; j < inp.length; j++) {
...
const rotate270 = (inp: string[][]): string[][] => {
  const output = makeOutput(inp);
  for (let i = 0; i < inp.length; i++) {
    for (let j = 0; j < inp.length; j++) {
...
const strToGrid = (str: string) => str.split('/').map((l) => l.split(''));

const gridToStr = (grid: string[][]) => grid.map((r) => r.join('')).join('/');

const addVariant = (grid: string[][], outcome: string) =>
  variants.push([gridToStr(grid), outcome]);
...
const getGrid = (ptrn: string, gridSize: number, row: number, col: number) => {
  let grid = '';
...
const rebuildPattern = (grids: string[][]) => {
...
const countPixels = (numSteps: number) => {
  let pattern = '.#./..#/###';
