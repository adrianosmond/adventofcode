...
const getBiodiversity = (state: string[][]) =>
  state
    .map((r) => r.join(''))
    .join('')
    .split('')
...
const getCount = (state: string[][], row: number, col: number) => {
  let count = 0;
...
const getKey = (state: string[][]) => state.map((r) => r.join('')).join('');

const nextMinute = (states: string[][][], currentState: number) => {
...
const makeLevels = (numLevels: number) =>
  new Array(numLevels)
    .fill(0)
    .map(() => new Array(size).fill(0).map(() => new Array(size).fill('.')));

// One of the ugliest functions I've ever written!
const getRecursiveCount = (
  state: string[][][],
  level: number,
  row: number,
  col: number,
