...
import { manhattan, strToIntArray } from '../utils/functions.ts';
...
let target: Coords | undefined;

const coordsToStr = (c: [number, number]) => `${c[0]},${c[1]}`;
const strToCoords = (str: string) => strToIntArray(str, ',') as Coords;

const getNeighbours = (coordStr: string) => {
...
const findShortestPath = (from: Coords, to: Coords) => {
...
const getCommand = ([fromX, fromY]: Coords, [toX, toY]: Coords) =>
  directions.findIndex((d) => d[0] === toX - fromX && d[1] === toY - fromY);

const getCommands = (path: Coords[]) => {
  const commands = [];
