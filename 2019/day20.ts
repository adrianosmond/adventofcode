...
const parseInput = (): [string[][], Portals, Position, Position] => {
  const maze = inputToCharGrid(input);

  let inner = null;
  const keys: Portals = {};
  const portals: Portals = {};
  let entrance: { row: number; col: number };
  let exit: { row: number; col: number };

  const addKey = (key: string, row: number, col: number, isInner = false) => {
    if (key === 'AA') {
      entrance = { row, col };
    } else if (key === 'ZZ') {
      exit = { row, col };
  ...
  if (inner === null) throw new Error('Something went wrong');
  ...
  // @ts-expect-error TS thinks that entrance and exit have not been assigned, but they have.
  if (!entrance || !exit) throw new Error('Something went wrong');

  return [maze, portals, entrance, exit];
};

const createMap = (maze: string[][], val: number) => {
  const map = [];
