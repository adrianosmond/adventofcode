...
const P2_CARD_VALUES: Record<Card, number> = {
  ...CARD_VALUES,
  J: 1,
};

const getScore = (counts: Record<number, number>) => {
...
const getCardCounts = (cards: number[], p2 = false) => {
  ...
  if (p2 && (counts[1] || 0) > 0) {
    ...
    Object.entries(counts).forEach(([card, count]) => {
      ...
      if (cardInt > highestCount) {
        highestCount = count!;
        mostCommonCard = cardInt;
      }
    });
...
const processHand = (hand: string, p2 = false): ProcessedHand => {
  const cards = hand
    .split('')
    .map((card) =>
      p2 ? P2_CARD_VALUES[card as Card] : CARD_VALUES[card as Card],
    );
...
const sortByHandValue = (
  [a]: [ProcessedHand, number],
  [b]: [ProcessedHand, number],
) => {
  if (a.score === b.score) {
...
const hands: [string, number][] = splitAndMapInputLines(input).map(
  ([hand, bid]) => [hand, parseInt(bid, 10)],
);

export const part1 = () =>
  hands
    .map<[ProcessedHand, number]>(([hand, bid]) => [processHand(hand), bid])
...
export const part2 = () =>
  hands
    .map<[ProcessedHand, number]>(([hand, bid]) => [
