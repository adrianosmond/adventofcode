...
const evaluateAdditionFirst = (equation: string) => {
  let withoutBrackets = equation;
  while (withoutBrackets.indexOf('(') >= 0) {
    withoutBrackets = withoutBrackets.replace(/\([^())]+\)/g, (match) =>
      evaluateAdditionFirst(match.replace(/[()]/g, '')).toString(),
    );
  }
