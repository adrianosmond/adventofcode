...
type Edge = {
  border: string;
  linkedTile: string | number | null;
  linkedEdge: string | number | null;
};
const tiles: Record<
  string,
  { rows: string[]; edges: [Edge, Edge, Edge, Edge] }
> = {};
...
const rotateTile = (tileId: string) => {
    ...
    for (let col = 0; col < Math.sqrt(Object.keys(tiles).length); col++) {
      if (col === 0) {
        if (row === 0) {
          ...
          tileKeys[row][col] = tiles[tileKeys[row - 1][col]].edges[2]
            .linkedTile as string;
        ...
        tileKeys[row][col] = tiles[tileKeys[row][col - 1]].edges[1]
          .linkedTile as string;
        let rotations = 0;
