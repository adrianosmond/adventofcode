...
import { getNeighbours, multilineStrToIntArrays } from '../utils/functions.ts';

type Coords = [number, number];

const input = readInput();
const coords = multilineStrToIntArrays(input, ',') as Coords[];
let grid: string[][];

const makeGrid = (gridSize: number, numBytes: number) => {
