...
import { getNeighboursWithDiagonals } from '../utils/functions.js';
...
const mapRobotsToQuadrants = (robots) => {
...
const clearGrid = (robots) => {
  robots.forEach((r) => {
    grid[r.pos[1]][r.pos[0]] = '.';
  });
};

const plotRobotsToGrid = (robots) => {
  robots.forEach((r) => {
    grid[r.pos[1]][r.pos[0]] = '#';
  });
};
...
const findChristmasTree = (robots) => {
  let time = 0;
  while (true) {
    time++;
    clearGrid(robots);
