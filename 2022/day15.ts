...
const beaconsSet = new Set<string>([]);
...
const sensors = input
  ...
  .map<[number, number, number]>(([x1, y1, x2, y2]) => {
...
const numBeaconsInTargetRowWithinRangeOfSensor = Array.from(beaconsSet)
  .map((b) => strToIntArray(b, ',') as [number, number])
  .filter(([, by]) => by === targetYRow)
  .map<number>((beacon) => {
    for (const [sx, sy, sd] of sensors) {
      if (manhattan(beacon, [sx, sy]) <= sd) {
        return 1;
      }
    }
    return 0;
