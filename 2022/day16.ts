...
const data = input
  ...
  .split('\n')
  .map((line) => line.split(', '))
  .map(([v, f, ...t]) => [v, parseInt(f, 10), ...t]) as [
  string,
  number,
  string,
][];

const network: Record<string, { flowRate: number; tunnels: string[] }> =
  Object.fromEntries(
    data.map(([v, flowRate, ...tunnels]) => [v, { flowRate, tunnels }]),
  );

const getShortestDistance = (start: string, end: string) => {
...
const makePaths = () => {
  const allPaths = [];
  const distances: Record<string, Record<string, number>> = Object.fromEntries(
