...
const directionEquals = (
  [y1, x1]: [number, number],
  [y2, x2]: [number, number],
) => y1 === y2 && x1 === x2;

const changeDirection = (direction: [number, number], rotation: string) => {
  if (rotation === 'L') {
...
const getScoreForDirection = ([y, x]: [number, number]) => {
  if (y === 0 && x === 1) return 0;
  if (y === 1 && x === 0) return 1;
  if (y === 0 && x === -1) return 2;
  if (y === -1 && x === 0) return 3;
...
const getFace = (grid: string[][], fy: number, fx: number) => {
  const face = [];
...
const getMapCoordinates = (face: Face, row: number, col: number) => [
  face.fy + row,
  face.fx + col,
];
