...
type Monkey = {
  items: number[];
  updateWorry: (val: number) => number;
  throwTo: (val: number) => number;
  inspections: number;
};

const input = readInput();
const getMonkeys = (): [Monkey[], number] => {
  let lcm = 1;
  const monkeys = input
    .replace(/Monkey \d+:\n/g, '')
    ...
    .map((m) => {
      ...
      if (amt === 'old') {
        updateWorry =
          op === '*' ? (val: number) => val * val : (val: number) => val + val;
      } else {
...
const simulateMonkeys = (
  numRounds: number,
  worryModifier = (worry: number) => worry,
) => {
