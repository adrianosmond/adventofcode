...
const blizzards: Blizzard[] = startingGrid
  .map((row, rowIdx) =>
    row.map((cell, colIdx) => {
      if (cell === 'v') {
        return {
...
const createMapArray = (numRepeats: number, val: number, grid: string[][]) => {
  const mapArray = [];
...
const entrance: Coords = [0, 1];
const exit: Coords = [grids[0].length - 1, grids[0][0].length - 2];

const findPath = (start: Coords, end: Coords, startTime: number) => {
  const openSet = [
    {
      row: start[0],
      col: start[1],
